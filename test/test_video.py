    ...
    for batch_idx, (frame_idx, data, target) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            for i in range(output.size(0)):
                ...
                detection_path = os.path.join('random_vid_detections', 'detections_'+ "title", frame_idx[i]) # TODO: change title
                current_dir = os.path.join('random_vid_detections', 'detections_'+ "title")
                if not os.path.exists('random_vid_detections'):
                    os.mkdir('random_vid_detections')
                if not os.path.exists(current_dir):
                    os.mkdir(current_dir)
